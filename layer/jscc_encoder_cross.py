import torch
import torch.nn as nn
import numpy as np
from timm.models.layers import trunc_normal_
from layer.layers import CrossAttentionBlock
from layer.jscc_encoder import RateAdaptionEncoder


class JSCCEncoderCross(nn.Module):
    """Cross-attention JSCC Encoder: rate/SNR embeddings as KV, image tokens as Q."""

    def __init__(self, embed_dim=256, depth=4, num_heads=8, mlp_ratio=4.,
                 norm_layer=nn.LayerNorm, rate_choice=[0, 128, 256],
                 input_resolution=(16, 16), depths=None, **kwargs):
        super().__init__()
        self.embed_dim = embed_dim
        self.rate_choice = rate_choice
        self.rate_num = len(rate_choice)
        self.register_buffer("rate_choice_tensor", torch.tensor(np.asarray(rate_choice)))

        if depths is not None:
            depth = sum(depths)
        if isinstance(num_heads, (list, tuple)):
            num_heads = num_heads[0]

        num_patches = input_resolution[0] * input_resolution[1]
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
        trunc_normal_(self.pos_embed, std=.02)

        self.layers = nn.ModuleList([
            CrossAttentionBlock(dim=embed_dim, num_heads=num_heads,
                                mlp_ratio=mlp_ratio, norm_layer=norm_layer)
            for _ in range(depth)
        ])
        self.norm = norm_layer(embed_dim)
        self.rate_adaption = RateAdaptionEncoder(embed_dim, rate_choice)

    def forward(self, z, cond, indexes):
        """
        Args:
            z: fused latent (B, C, H, W)
            cond: conditioning tokens (B, K, D) — rate + SNR embeddings
            indexes: rate indexes (B*H*W,) for RateAdaptionEncoder
        Returns:
            s_masked, mask_BCHW
        """
        B, C, H, W = z.size()
        x_BLC = z.flatten(2).permute(0, 2, 1)
        x_BLC = x_BLC + self.pos_embed[:, :H * W, :]
        for layer in self.layers:
            x_BLC = layer(x_BLC, cond)
        x_BLC = self.norm(x_BLC)
        x_BCHW = x_BLC.reshape(B, H, W, C).permute(0, 3, 1, 2)
        s_masked, mask_BCHW = self.rate_adaption(x_BCHW, indexes)
        return s_masked, mask_BCHW

    def update_resolution(self, H, W):
        pass
